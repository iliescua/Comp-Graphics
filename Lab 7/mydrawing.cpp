...
void MyDrawing::paint(GraphicsContext* gc) { im.draw(gc, &vc); }
...
void MyDrawing::keyDown(GraphicsContext* gc, unsigned int keycode) {
  switch (keycode) {
    ...
    case 0x0063:  // Clears the canvas with c
      this->clear(gc);
      break;
    case 0x0070:  // Set triangle point with p
      inTri = true;
      break;
    case 0x0072:  // Rotate with r
      vc.rotate();
      gc->clear();
      im.draw(gc, &vc);
      break;
    case 0x0073:  // Scale with s
      vc.scale();
      gc->clear();
      im.draw(gc, &vc);
      break;
