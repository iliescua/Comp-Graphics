...
X11Context::X11Context(unsigned int sizex = 400, unsigned int sizey = 400,
  ...
  window = XCreateSimpleWindow(display, DefaultRootWindow(display), 0, 0, sizex,
                               sizey, 0, 0, bg_color);
...
void X11Context::runLoop(DrawingBase* drawing) {
  ...
  while (run) {
    ...
    // Mouse Button Up
      ...
      drawing->mouseButtonUp(this, e.xbutton.button, e.xbutton.x, e.xbutton.y);
